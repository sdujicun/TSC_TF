...
def featureNumExperiemnt(dataset='Beef',segAvgLen=10,nb_epochs=2000):
    ...
    for i in range(6):
        ...
        fcn.fit(x_train, x_test, Y_train, Y_test,nb_epochs=nb_epochs)
        ...
        print('recall', recall)
        df=df.append({'dataset':dataset,'featureNum':featureNum,'precision':precision,'accuracy':accuracy,'recall':recall},ignore_index=True)
...
if __name__ == '__main__':
    ...
    #datasets=['Coffee','MoteStrain','Trace','MedicalImages','TwoPatterns','Mallat']
    datasets=['TwoPatterns','Mallat','Coffee','MoteStrain','Trace']
    for dataset in datasets:
        for itr in range(10):

            df=pd.DataFrame(columns=['dataset','featureNum','precision','accuracy','recall'])
            df_dataset=featureNumExperiemnt(dataset=dataset)
            df=df.append(df_dataset)
            resultFileName="..\\result\\FeatureNumTest_"+dataset+"_"+str(itr)+".csv"
